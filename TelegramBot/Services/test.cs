...
namespace TelegramBot.Services
{
    ...
    //        var user = await _dbContext.Users.FirstOrDefaultAsync(u => u.TelegramId == chatId);

    //        // Если пользователя нет, предлагаем ему оплатить подписку
    //        if (user == null)
    //        {
    //            await _botClient.SendTextMessageAsync(chatId, "Пожалуйста, оплатите подписку, чтобы начать использовать бота.");
    //            return;
    //        }
    ...
    //        {
    //            if (user.Status == "awaiting_main_video")
    //            {
    //                await HandleMainVideoUpload(user, message);
    //            }
    //            else if (user.Status == "awaiting_chroma_video")
    //            {
    //                await HandleChromaVideoUpload(user, message);
    //            }
    //        }
    ...
    //        if (user.LastMessageId.HasValue)
    //        {
    //            try
    //            {
    //                await botClient.DeleteMessageAsync(chatId, (int)user.LastMessageId.Value);
    //            }
    //            catch (Exception ex)
    //            {
    ...
    //        {
    //            await HandleUploadVideoCallback(user);
    //        }
    //        else if (callbackData == "cancel_upload")
    //        {
    //            await HandleCancelUploadCallback(user);
    //        }
    ...
    //        {
    //            await HandleCheckPaymentStatusCallback(user); // Проверка статуса платежа
    //        }
    //    }
    //}
